...
def plot_data_error(data, indices, indices_2, column_names, column_names_2):
    times = data[:, 0]  # get time stamp
    times -= times[0]  # shift to zero
    ...
    for i, index in enumerate(indices):
        plt.subplot(len(indices), 1, i + 1)
        plt.plot(times, data[:, index], label=column_names[i])
        plt.plot(times, data[:, indices_2[i]], label=column_names_2[i])
        # plt.xlabel('Time (s)')
        plt.ylabel(column_names[i])
        # plt.title(f'Time Series Plot for {column_names[i]}')
        plt.legend()
...
def main():
    ...
    plot_data_error(selected_data, [13,14,15,16,17,18], [37,38,39,40,41,42], ['l1', 'l2', 'l3', 'l4', 'l5', 'l6'], ['cmd_l1', 'l2', 'l3', 'l4', 'l5', 'l6'])
