...
class parallel_ankle {
  public:
    ...
    bool setRef(const VectorXd &ankleOrienRef,
                const VectorXd &ankleOmegaRef,
                const VectorXd &ankleTorRef);
    bool calcIK();
    bool getMotorCmd(VectorXd &qDes, VectorXd &qDotDes, VectorXd &tauDes);

    Matrix3d radian_to_rotation_matrix(double roll, double pitch);
    Matrix3d single_radian_rotation_matrix(double pitch);
